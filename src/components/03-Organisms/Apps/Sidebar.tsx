...
import { faHouse, faCalendarAlt, faHeart, faSyringe, faChartColumn, faUsers, faScrewdriverWrench, faTimes } from "@fortawesome/free-solid-svg-icons";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    const menuIndex = menus.findIndex((menu) => {
      if (menu.submenus) {
        return menu.submenus.some((submenu) => submenu.url === pathname);
      }
  ...
  return (
    <Fragment>
      ...
      <aside
        className={`bg-[#282733] text-white w-64 h-screen fixed top-0 left-0 transform transition-transform duration-300 ease-in-out z-10 ${
      ...
      >
        ...
        <ul>
          {menus.map((menu, index) => {
            const isActive = pathname === menu.url || (menu.submenus && menu.submenus.some((submenu) => submenu.url === pathname));
            ...
            return (
              ...
              >
                <div
                  className={`flex justify-between items-center cursor-pointer py-3 px-7 group ${isActive ? "bg-red-600" : "hover:bg-gray-700"}`}
                ...
                >
                  {menu.url ? (
                    ...
                    >
                      <span className="group-hover:text-blue-500">{menu.icon}</span>
                  ...
                  {menu.submenus && <span className={`transform transition-transform duration-300 ${openMenu === index ? "rotate-90" : ""} group-hover:text-blue-500`}>&rsaquo;</span>}
                </div>

                {menu.submenus && (
                  <ul className={`mt-2 overflow-hidden transition-[max-height] duration-300 ease-in-out ${openMenu === index ? "max-h-96" : "max-h-0"}`}>
                    {menu.submenus.map((submenu, subIndex) => (
                      <li key={subIndex}>
                        <div className={`py-3 pl-10 text-sm group hover:bg-gray-700 ${pathname === submenu.url ? "text-white bg-gray-700" : "text-gray-400"}`}>
                          <Link
                            href={submenu.url}
                            className="flex items-center hover:text-white"
                          >
                            <span className="mr-4 group-hover:text-blue-500">&bull;</span>
                            {submenu.label}
                          </Link>
                        </div>
                      </li>
